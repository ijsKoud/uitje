...
import { transactionCreateFormHandler } from "./server-actions";
...
const TransactionDialog: React.FC<Props> = ({ participants, user, uitjeId }) => {
	...
	async function onSubmit(values: z.infer<ReturnType<typeof getTransactionSchema>>) {
		try {
			await transactionCreateFormHandler(values, uitjeId);
			router.refresh();
			form.reset({ amount: 0, date: new Date(), description: "", paidBy: user, participants: [], type: "total" });
			setOpen(false);
		} catch (err) {
			console.error(err);
		...
		<Dialog open={open} onOpenChange={setOpen}>
			<DialogTrigger asChild>
				<Button variant="default">
					<WalletIcon className="mr-2 h-4 w-4" /> Add transaction
				</Button>
			</DialogTrigger>
